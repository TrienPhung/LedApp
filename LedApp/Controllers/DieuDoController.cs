...
{
    [Authorize]
    public class DieuDoController : Controller
    {
        private readonly ApplicationDBContext _context;
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IHubContext<SignalServer> _hubContext;
        private readonly ILogger<DieuDoController> _logger;

        public DieuDoController(
            ApplicationDBContext context,
            IHttpClientFactory httpClientFactory,
            IHubContext<SignalServer> hubContext,
            ILogger<DieuDoController> logger)
        {
            _context = context;
            ...
            try
            {
                ...
                var result = xes.Select(x => new
                {
                    ...
                    chuyenHienTai = x.ChuyenHienTai == null ? null : new
                    {
                        ...
                        lanThu = x.ChuyenHienTai.LanThu,
                        hangHoas = x.ChuyenHienTai.HangHoas,
                        thoiGianDen = x.ChuyenHienTai.ThoiGianDen  // ← thêm để payload dùng
                    },
                    daPhanCong = bienSoDaPhan.Contains(x.BienSo),
                    cuaNhapId = daPhanList.FirstOrDefault(d => d.BienSoXe == x.BienSo)?.CuaNhapId  // ← thêm
                });
                return Ok(result);
            }
            catch (Exception ex)
